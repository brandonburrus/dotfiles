...
**Workflow pattern** — use when the skill guides a repeatable process that produces an artifact or executes a sequence of steps (installing something, searching memory, creating a file, running a release):
...
| `install-mcp` | Workflow, detailed | Workflow with branching logic and config examples |
| `mcp-setup-instruction` | Workflow, detailed | Workflow that produces a structured file with frontmatter |
| `persuasive-writeup` | Style/output | Comprehensive writing style guide with tone rules |
| `educative-writeup` | Style/output | Writing style guide with teaching-specific techniques |
